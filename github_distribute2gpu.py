...
2 GPUs, the DFIX method itself is mainly omitted (and at the end was not used for this purpose).
...
def grad(trainable_variables, x, t, indices_zero_one):
    g = dnn.grad1(x, t, indices_zero_one, trainable_variables, trainable_variables[0])
    g = tf.reshape(g, (tf.shape(g)[0] * tf.shape(g)[1], 1))
    for v in trainable_variables[1:]:
        bb = dnn.grad1(x, t, indices_zero_one, trainable_variables, v)
        sh1 = tf.shape(bb)[0]
        sh2 = tf.shape(bb)[1]
...
############ hessian #############
# not used here, but if needed, this is how to use distributed_nn_functions to
# create the whole hessian matrix

def Hes(train, x, t, indices_zero_one):
    m = 2  # how to divide trains and hence the hessian
    part_of_trains = train[:m]
    indices = [ind for ind in range(m)]
    pr = dnn.hes_part(x, t, indices_zero_one, train, part_of_trains, indices)

    part_of_trains = train[m:]
    indices = [ind for ind in range(m, len(train))]
    dr = dnn.hes_part(x, t, indices_zero_one, train, part_of_trains, indices)
    ...
    hes = (
        tf.transpose(tf.linalg.band_part(hes, 0, -1))
        + tf.linalg.band_part(hes, 0, -1)
        - tf.linalg.band_part(hes, 0, 0)
    )
    return hes
...
def test_update(trainable_variables, X, d, a):
    with tf.device("/gpu:0"):
        ...
        indices = [ind for ind in range(m)]
        first = dnn.hes_part(
            x, t, indices_zero_one, trainable_variables, part_of_trains, indices
        )
...
x = input_x_t_ind[0]
t = input_x_t_ind[1]
indices_zero_one = input_x_t_ind[2]
trainable_variables = dnn.create_weights(15, 15, 0, 0.1)

############### DFIX: main algorithm, returning trainable vars #####################
...
limits = tf.constant([0, 15, 30, n])  # nods
#%%
...
deg = tf.reduce_sum(
    tf.sign(W), axis=0
)  # 1/0; nod with the largest number of neighbours
...
while k < 10 and exit_criteria == 0:
    start = time.time()
